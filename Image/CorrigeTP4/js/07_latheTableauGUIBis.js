...
// fin du plan du sol
 ...
 let M2a = new THREE.Vector3(2,-2,0);
 let M2b = new THREE.Vector3(1,-4,0);
 let M3 = new THREE.Vector3(0.25,-4,0);
 ...
 let dimPt=0.05;  
 var tabP= new Array(4);  //pts de controle de la premiere courbe 
 var tabP1= new Array(5); //pts de controle de la seconde courbe
 for (var k=0;k<tabP.length;k++){
   tabP[k]= new THREE.Vector3(0,0,0);
 }
 for (var k=0;k<tabP1.length;k++){
   tabP1[k]= new THREE.Vector3(0,0,0);
 }
 tabP[0].copy(P0);tabP[1].copy(P1);
 tabP[2].copy(P2);tabP[3].copy(P3); 
 tabP1[0].copy(M0);tabP1[1].copy(M1); 
 tabP1[2].copy(M2a);tabP1[3].copy(M2b);
 tabP1[4].copy(M3);
 for (var k=0;k<tabP.length-1;k++)
   tracePt(scene, tabP[k], "#008888",dimPt,true);
 // point de recollement
 tracePt(scene, tabP1[0], "#000000",dimPt,true);
 let ptM1 = tracePt(scene, tabP1[1], "#00FF00",dimPt,false);
 for (var k=2;k<tabP1.length;k++)
   tracePt(scene, tabP1[k], "#008888",dimPt,true);
 let lathe1 = latheBezTab(scene,nbPtCB,nbePtRot,tabP,"#884400",0.95,false); 
 let lathe2 = latheBezTab(scene,nbPtCB,nbePtRot,tabP1,"#880044",0.95,false);
 ...
 let guiCoefG1 = gui.addFolder("Coefficient pour la jointure G1");
 guiCoefG1.add(menuGUI,"coefG1",-15,15).onChange(function () {
   vTan2.copy(origine);
   vTan2.addScaledVector(vP2P3,menuGUI.coefG1);
   tabP1[1].copy(origine);
   tabP1[1].addVectors(M0,vTan2);
   scene.remove(lathe1);
   // efface lathe2 et ptM1 s'ils existent
   if (lathe2) scene.remove(lathe2);
   if (ptM1) scene.remove(ptM1);
   if (cbeBez2) scene.remove(cbeBez2);
   lathe1 = latheBezTab(scene,nbPtCB,nbePtRot,tabP,"#884400",0.95,false); 
   lathe2 = latheBezTab(scene,nbPtCB,nbePtRot,tabP1,"#880044",0.75,true);
   ptM1=tracePt(scene, tabP1[1], "#00FF00",dimPt,false);
   posCamera();
   scene.add(lathe1);
   scene.add(lathe2);
   scene.add(ptM1);
   cbeBez2 = traceBezierTab(tabP1,100,"#88FF99",epai);
   scene.add(cbeBez2);
   document.getElementById("result").innerHTML+="<br />"+vTan2.x;
   document.getElementById("result").innerHTML+=" ;  "+vTan2.y;
   document.getElementById("result").innerHTML+=" ;  "+vTan2.z+"<br />";
   document.getElementById("result").innerHTML+="<br /> menuGUI.coefG1 : "+menuGUI.coefG1+"<br />";
   document.getElementById("result").innerHTML+="<hr />"+tabP1[1].x;
   document.getElementById("result").innerHTML+=" ;  "+tabP1[1].y;
   document.getElementById("result").innerHTML+=" ;  "+tabP1[1].z+"<br />";
 });

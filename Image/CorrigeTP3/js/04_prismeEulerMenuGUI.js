...
function faceEuler(MaScene,R,n,k,h,coulArete,epai,CoulPt,dimPt){
 ...
 //couvercle du prisme
 // couvercle de la boite de conserve
 couvercle.vertices = [ PtBC, PtAC, CenHaut];//doublons pour ne pas faire tourner les sommets des cotes.
 ...
 // fond du prisme
 fond.vertices = [ PtDB, PtCB, CenBas];//doublons pour ne pas faire tourner les sommets des cotes.
 ...
 let faceFond = new THREE.Mesh( fond, coteMaterial );
 MaScene.add( faceFond );
 let faceCouvercle = new THREE.Mesh( couvercle, coteMaterial );
 MaScene.add( faceCouvercle );
 let faceCote = new THREE.Mesh( cote, coteMaterial );
 MaScene.add( faceCote );
}//fin fonction faceEuler
...
function init(){
 ...
 let CoulPt="#BB88BB";
 let dimPt=0.05;
 for(let k=0;k<n;k++) faceEuler(scene,R,n,k,h,coulArete,epai,CoulPt,dimPt);
